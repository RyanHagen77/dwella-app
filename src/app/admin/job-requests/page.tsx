 ...
 *
 * Location: app/admin/job-requests/page.tsx
 */
...
export default async function JobRequestsPage({
  ...
  const [jobRequests, counts] = await Promise.all([
    ...
    Promise.all([
      ...
      prisma.jobRequest.count({ where: { status: "DECLINED" } }),
      prisma.jobRequest.count({ where: { status: "CANCELLED" } }),
    ]),
  ]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className={"mt-1 " + textMeta}>{jobRequests.length} requests</p>
        </div>
      </div>

      <div className="flex flex-wrap gap-2">
        <StatusTab href="/admin/job-requests" label="All" count={pendingCount + acceptedCount + completedCount + declinedCount + cancelledCount} active={statusFilter === "all"} icon={ClipboardList} color="white" />
        <StatusTab href="/admin/job-requests?status=pending" label="Pending" count={pendingCount} active={statusFilter === "pending"} icon={Clock} color="amber" />
        <StatusTab href="/admin/job-requests?status=accepted" label="Accepted" count={acceptedCount} active={statusFilter === "accepted"} icon={CheckCircle} color="emerald" />
        <StatusTab href="/admin/job-requests?status=completed" label="Completed" count={completedCount} active={statusFilter === "completed"} icon={CheckCircle} color="blue" />
        <StatusTab href="/admin/job-requests?status=declined" label="Declined" count={declinedCount} active={statusFilter === "declined"} icon={XCircle} color="red" />
        <StatusTab href="/admin/job-requests?status=cancelled" label="Cancelled" count={cancelledCount} active={statusFilter === "cancelled"} icon={Ban} color="gray" />
      </div>
  ...
  return (
    <div className={glass + " flex flex-col"}>
      ...
      <div className="space-y-2">
        <Link href={"/admin/homes/" + job.home.id} className="flex items-center gap-2 group">
          <MapPin size={14} className="text-white/50" />
          <span className="text-sm text-white/80 group-hover:text-emerald-400">{job.home.address}, {job.home.city}</span>
        </Link>

        <Link href={"/admin/users/" + job.homeowner.id} className="flex items-center gap-2 group">
          <User size={14} className="text-white/50" />
          <span className="text-sm text-white/80 group-hover:text-emerald-400">{job.homeowner.name || job.homeowner.email}</span>
        </Link>

        {job.contractor && (
          <Link href={"/admin/users/" + job.contractor.id} className="flex items-center gap-2 group">
            <Briefcase size={14} className="text-white/50" />
            <span className="text-sm text-white/80 group-hover:text-blue-400">
              {job.contractor.proProfile?.businessName || job.contractor.name || job.contractor.email}
            </span>
          </Link>
        )}
      </div>
