...
export function DocumentServiceClient({ connectedHomes }: DocumentServiceClientProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.homeId || !form.serviceType.trim()) {
      alert("Property and document-completed-service-submissions type are required");
      return;
    }
    ...
    try {
      // Step 1: Create document-completed-service-submissions record WITHOUT files first
      ...
      if (!createResponse.ok) {
        ...
        throw new Error(error.error || "Failed to create document-completed-service-submissions record");
      }

      const { serviceRecord: serviceRecord } = await createResponse.json();
      const recordId = serviceRecord.id;
      ...
      // Step 3: Update document-completed-service-submissions record with file URLs
      ...
      // Redirect to the document-completed-service-submissions record detail page
      router.push(`/pro/contractor/service-records/${recordId}`);
    } catch (error) {
      console.error("Error documenting document-completed-service-submissions:", error);
      alert(error instanceof Error ? error.message : "Failed to document document-completed-service-submissions. Please try again.");
    } finally {
      setSaving(false);
      setUploading(false);
    }
