...
export function EditReminderModal({ open, onClose, reminder, homeId }: Props) {
  ...
  // Initialize form with current reminder data
  ...
  // Reset form when reminder changes or modal opens
  useState(() => {
    if (open) {
      setForm({
        title: reminder.title,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // First update the reminder
      const res = await fetch(`/api/home/${homeId}/reminders/${reminder.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: form.title,
          dueAt: form.dueAt, // Send as YYYY-MM-DD string
          note: form.note || null,
        }),
      });
