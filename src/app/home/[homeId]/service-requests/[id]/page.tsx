...
export default async function ServiceRequestDetailPage({ params }: PageProps) {
  ...
  // Verify ownership
  if (
    serviceRequest.homeId !== homeId ||
    serviceRequest.homeownerId !== session.user.id
  ) {
  ...
  const addrLine = [serviceRequest.home.address, serviceRequest.home.city, serviceRequest.home.state]
    .filter(Boolean)
    .join(", ");
          ...
          <div className="space-y-6 lg:col-span-2">
              ...
              <dl className="space-y-3">
                ...
                {(serviceRequest.budgetMin || serviceRequest.budgetMax) && (
                  <div className="flex gap-3">
                    ...
                    <dd className="text-white">
                      $
                      {Number(serviceRequest.budgetMin || 0).toLocaleString()} - $
                      {Number(serviceRequest.budgetMax || 0).toLocaleString()}
                    </dd>
                  </div>
                )}
                {serviceRequest.desiredDate && (
                  <div className="flex gap-3">
                    ...
                    <dd className="text-white">
                      {format(
                        new Date(serviceRequest.desiredDate),
                        "MMM d, yyyy"
                      )}
                    </dd>
                  </div>
                ...
                {serviceRequest.respondedAt && (
                  <div className="flex gap-3">
                    ...
                    <dd className="text-white">
                      {format(
                        new Date(serviceRequest.respondedAt),
                        "MMM d, yyyy 'at' h:mm a"
                      )}
                    </dd>
                  </div>
            ...
            {serviceRequest.quote && (
              <section className={glass}>
                ...
                {serviceRequest.quote.items &&
                  serviceRequest.quote.items.length > 0 && (
                    <div className="space-y-2">
                      <h3 className="text-sm font-semibold text-white/80">
                        Line Items:
                      </h3>
                      <div className="overflow-hidden rounded-lg border border-white/10 bg-white/5">
                        {serviceRequest.quote.items.map((item) => (
                          <div
                            key={item.id}
                ...
                {serviceRequest.quote.expiresAt && (
                  <p className={`mt-4 text-xs ${textMeta}`}>
                    Quote expires:{" "}
                    {format(
                      new Date(serviceRequest.quote.expiresAt),
                      "MMM d, yyyy"
                    )}
                  </p>
                )}
              ...
              <section className={glass}>
                ...
                <div className="flex items-start gap-3">
                  {serviceRequest.contractor.image && (
                    <div className="relative h-16 w-16 flex-shrink-0 overflow-hidden rounded-full">
                      <Image
                        ...
                        alt={serviceRequest.contractor.name || "Contractor"}
                        fill
                  ...
                  <div className="flex-1">
                    ...
                    {serviceRequest.contractor.proProfile?.rating && (
                      <p className={`text-sm ${textMeta}`}>
                        ⭐{" "}
                    ...
                    {serviceRequest.contractor.proProfile?.completedJobs && (
                      <p className={`text-sm ${textMeta}`}>
                        {serviceRequest.contractor.proProfile.completedJobs} jobs
                        completed
                      </p>
                    )}
