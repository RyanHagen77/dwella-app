...
export default function CompletedWorkSubmissionsClient({
  homeId,
  homeAddress,
  pendingWork,
  serviceRequests,
...
function AllTab({
  ...
  const hasAny = serviceRequests.length > 0 || pendingWork.length > 0;
  ...
  return (
    <div className="space-y-8">
      ...
      {serviceRequests.length > 0 && pendingWork.length > 0 && (
                ...
                {service.workRecord && (
                  <div className="mt-3 rounded-lg border border-green-500/30 bg-green-500/10 p-3">
                    ...
                    <p className="text-sm text-white/80">
                      {new Date(
                        service.workRecord.workDate
                      ).toLocaleDateString()}{" "}
                      • {service.workRecord.status}
                    </p>
                  </div>
                )}
              </div>
...
function PendingWorkTab({
  ...
  async function handleApprove(workId: string) {
    try {
      const res = await fetch(
        `/api/home/${homeId}/completed-work-submissions/${workId}/approve`,
        {
          method: "POST",
        }
      );
  ...
  async function handleReject(workId: string) {
    ...
    try {
      const res = await fetch(
        `/api/home/${homeId}/completed-work-submissions/${workId}/reject`,
        {
          method: "POST",
        }
      );
